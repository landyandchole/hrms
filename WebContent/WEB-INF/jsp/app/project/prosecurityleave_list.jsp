...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">							
						<!-- 检索  -->
						<form action="security/listProsecurityleave.do" method="post" name="Form" id="Form">
					 ...
					 <div  style="overflow-x: scroll;">
                      <div style="width:1300px">
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty prosecurityleave}">
									<c:if test="${QX.cha == 1 }">
									<c:forEach items="${prosecurityleave}" var="var" varStatus="vs">
										<tr>
										
											<td>
											 <a id="membera${var.PROJECT_ID}" class="btn btn-xs btn-info"  onclick="proMember(this,'${var.PROJECT_ID}','${var.PC_LEAVEID}',
											 '${pd.pMember}','${pd.projectname}','${pd.projectmanager}','${pd.pcnumber}','${pd.admissioncharge}','${pd.exitcharge}','${pd.applicant}');">
														+
													</a>
											</td>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.PRO_ID}" id="${var.APPLICANT_NO}" class="ace" /><span class="lbl"></span></label>
								...
								<thead id="div${var.PC_LEAVEID}" style="display:none;">
								   <tr>
								   ...
								   <th class="center" style="border:none;"></th>
									...
									<tbody id="J_TbData${var.PC_LEAVEID}">
                                     </tbody>									
									</c:forEach>
									</c:if>
									<c:if test="${QX.cha == 0 }">
	...
	<script type="text/javascript">
		...
		function proMember(obj,id,pc_leaveid,proid,projectname,projectmanager,pcnumber,admissioncharge,exitcharge,applicant){
			 ...
			 if (obj.innerHTML.indexOf('+') != -1) {	
					$.ajax({
						...
						url: '<%=basePath%>security/hasproSecurity.do?tm='+new Date().getTime()+
								'&PC_LEAVEID='+pc_leaveid+'&projectname='+projectname+'&projectmanager='+projectmanager+
								'&pcnumber='+pcnumber+'&PC_STATE='+PC_STATE+'&admissioncharge='+admissioncharge+'&exitcharge='+exitcharge+'&applicant='+applicant,
						dataType:'json',
						cache: false,
						success: function(data){
								document.getElementById("div"+pc_leaveid+"").style.display="";
								$("#J_TbData"+pc_leaveid+"").html("");
							  $.each(data.varList, function(i, dvar){
								 ...
								 var $trTemp = $("<tr></tr>");
					                ...
					                $trTemp.append('<td style="border:none;"></td>');
					                $trTemp.append('<td ><label class="pos-rel"><input type="checkbox" name="ids" value='+dvar.PRO_ID+' id="${'+dvar.APPLICANT_NO+'}" class="ace" /><span class="lbl"></span></label>');
					                ...
					                //$trTemp.append('<td class="center">'<a class="btn btn-xs btn-success" title="编辑" onclick="edit("+dvar.PRO_ID+","+dvar.PC_LEAVEID+","+dvar.PROJECT_ID+");"><i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i></a>'</td>');
					                //var   $tem=$("<td></td>");
					                //$a=$("<a class='btn btn-xs btn-success' title='编辑' onclick='edit("+dvar.PRO_ID+","+dvar.PC_LEAVEID+","+dvar.PROJECT_ID+");'><i class='ace-icon fa fa-pencil-square-o bigger-120' title='编辑'></i></a>");					                
					                //$trTemp.append($a);
					                $trTemp.append('<td class="center"><c:if test="${QX.edit == 1 }"><a class="btn btn-xs btn-success" title="编辑" onclick="edit(/'+dvar.PRO_ID+'/,/'+dvar.PC_LEAVE_ITEMID+'/,/'+dvar.PROJECT_ID+'/);"><i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i></c:if></a>'
					                +"&nbsp;"+'<a class="btn btn-xs btn-info" title="退场申请" onclick="startApply(/'+dvar.PC_LEAVEID+'/,/'+dvar.PROJECT_ID+'/);"><span id="discount" style="width:20px;height:20px;">退场</span></a>'
					                +"&nbsp;"+'<c:if test="${QX.del == 1 }"><a class="btn btn-xs btn-danger" onclick="del(/'+dvar.PRO_ID+'/);"><i class="ace-icon fa fa-trash-o bigger-120" title="删除"></i></a></c:if>')
					                $trTemp.appendTo("#J_TbData"+pc_leaveid+"");
						          });
							   document.getElementById("J_TbData"+pc_leaveid+"").style.display="";
							   obj.innerHTML = '-';
						}
					});
				...
				document.getElementById("div"+pc_leaveid+"").style.display="none";
				document.getElementById("J_TbData"+pc_leaveid+"").style.display="none";
				obj.innerHTML = '+';
			}  
		...
		//修改
		function edit(Id,PC_LEAVE_ITEMID,PROJECT_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>security/goEdit.do?PRO_ID='+Id+'&ID='+PC_LEAVE_ITEMID+'&PROJECT_ID='+PROJECT_ID;
			 diag.Width = 450;
			 diag.Height = 410;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//安全检查
	    function safe(pro_Id,pc_state,pc_no){
	   	      top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="安全检查";
			 diag.URL = '<%=basePath%>pccheck/list.do?PC_ID='+pro_Id+'&PC_STATE='+pc_state+'&PC_NO='+pc_no;
			 diag.Width = 1200;
			 diag.Height =800;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
