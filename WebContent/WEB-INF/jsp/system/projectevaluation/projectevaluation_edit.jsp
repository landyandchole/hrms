...
<body class="no-skin">
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<form action="projectev/${msg}.do" name="Form" id="Form"
								method="post">
								...
								<div id="zhongxin" style="padding-top: 13px;">
									<table id="table_report"
										...
										<tr>
											...
											<td colspan="10"><input type="text"
												...
												value="${pd.QUESTION_NAME}" maxlength="100"
										...
										<tr class="mark_answer">
											...
											<td><input type="text" id="1" name="ANSWER_NAME[0]"
												maxlength="50" value="${pd.ANSWER_NAME}"
												placeholder="这里输入答案" title="答案" style="width: 98%;" /></td>
											<td	style="width: 75px; text-align: right; padding-top: 13px;">分值:</td>
											<td><input type="text" id="2" name="ANSWER_MARK[0]"
												value="${pd.ANSWER_MARK}" maxlength="50"
                                    ...
                                    <tr class="mark_answer">
											<td style="width: 75px; text-align: right; padding-top: 13px;">答案:</td>
											<td><input type="text" id='1' name="ANSWER_NAME"
												maxlength="50" value="${pd.ANSWER_NAME}"
												placeholder="这里输入答案" title="答案" style="width: 98%;" /></td>
											<td	style="width: 75px; text-align: right; padding-top: 13px;">分值:</td>
											<td><input type="text" id="2" name="ANSWER_MARK"
												value="${pd.ANSWER_MARK}" maxlength="50"
												placeholder="这里输入分值" title="请输入数字" style="width: 70%;" onKeyUp="shuru(this);"  />
												</td>
                                    </c:if>
	...
	<script type="text/javascript">
	  ...
	  $(document).ready(function(){
		 ...
		 function addTable() {
			 $("#table_report").append('<tr class="mark_answer" ><td style="width: 75px; text-align: right; padding-top: 13px;">答案:</td><td><input type="text" id="1" name="ANSWER_NAME['+ trcount +']" maxlength="50" placeholder="这里输入答案" title="答案" style="width:98%;"/></td>	<td style="width:75px;text-align: right;padding-top: 13px;">分值:</td><td><input type="text" id="2" name="ANSWER_MARK['+ trcount +']" maxlength="50"  placeholder="这里输入分值" title="请输入数字" style="width:70%;" onKeyUp="shuru(this);" /><td><input type="button" value="增加" class="addTable"/><input type="button" value="删除" class="deleteTable"/></td></tr>');
	 ...
	 function save(){
				var namearr = [];
		 		...
		 		$(".mark_answer").each(function (index,element) {
		 			namearr.push($(element).find('[name=ANSWER_NAME]').val());
		 			markarr.push($(element).find('[name=ANSWER_MARK]').val());
		 		});
					$("#mark_answer_a").val(namearr.join('\n'))
					$("#mark_answer_q").val(markarr.join('\n'))
					
				if($("#QUESTION_TYPE").val()=="请选择"){
					$("#QUESTION_TYPE").tips({
						side:3,
			            msg:'请输入类别',
			            bg:'#AE81FF',
