...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td style="width:30px;"><a onclick="roleButton('${var.DEPARTMENT_ID }','add_qx');" class="btn btn-warning btn-mini" title="分配新增权限"><i class="ace-icon fa fa-wrench bigger-110 icon-only"></i></a></td>
											<td style="width:30px;"><a onclick="roleButton('${var.DEPARTMENT_ID }','del_qx');" class="btn btn-warning btn-mini" title="分配删除权限"><i class="ace-icon fa fa-wrench bigger-110 icon-only"></i></a></td>
											<td style="width:30px;"><a onclick="roleButton('${var.DEPARTMENT_ID }','edit_qx');" class="btn btn-warning btn-mini" title="分配修改权限"><i class="ace-icon fa fa-wrench bigger-110 icon-only"></i></a></td>
											<td style="width:30px;"><a onclick="roleButton('${var.DEPARTMENT_ID }','cha_qx');" class="btn btn-warning btn-mini" title="分配查看权限"><i class="ace-icon fa fa-wrench bigger-110 icon-only"></i></a></td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-purple" onclick="editRights('${var.DEPARTMENT_ID }');"><i class="icon-pencil"></i>菜单权限</a>
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a class="btn btn-mini btn-purple" onclick="editRights('${var.DEPARTMENT_ID }');"><i class="icon-pencil"></i>菜单权限</a>
	...
	<script type="text/javascript">
		...
		//按钮权限(增删改查)
		function roleButton(DEPARTMENT_ID,msg){
			top.jzts();
			if(msg == 'add_qx'){
			 ...
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = '<%=basePath%>department/b4Button.do?DEPARTMENT_ID='+DEPARTMENT_ID+'&msg='+msg;
			 diag.Width = 330;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		//菜单权限
		function editRights(DEPARTMENT_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = '<%=basePath%>department/menuqx.do?DEPARTMENT_ID='+DEPARTMENT_ID;
			 diag.Width = 320;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
