package com.hand.controller.system.buttonrights;
...
public class ButtonrightsController extends BaseController {
	...
	public Object updateRolebuttonrightd()throws Exception{
		...
		if(null != buttonrightsService.findById(pd)){	//判断关联表是否有数据 是:删除/否:新增
			...
			FHLOG.save(Jurisdiction.getUsername(), "删除按钮权限"+pd,"SYS_ROLE_FHBUTTON","BUTTON_ID",pd.getString("BUTTON_ID"));
		}else{
			pd.put("RB_ID", this.get32UUID());	//主键
			buttonrightsService.save(pd);		//新增
			FHLOG.save(Jurisdiction.getUsername(), "新增按钮权限pd"+pd,"SYS_ROLE_FHBUTTON","RB_ID",pd.getString("RB_ID"));
		}
