package com.hand.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public Object getLevels(){
		...
		try{
			String roleName = "职位组";
			List<Role> roleList = roleService.listAllRolesByName(roleName);
			List<PageData> pdList = new ArrayList<PageData>();
			for(Role d :roleList){
				PageData pdf = new PageData();
				pdf.put("ROLE_ID", d.getROLE_ID());
				pdf.put("ROLE_NAME", d.getROLE_NAME());
				pdList.add(pdf);
			}
			map.put("roleList", pdList);	
		} catch(Exception e){
			errInfo = "error";
			logger.error(e.toString(), e);
		}
	...
	public ModelAndView add()throws Exception{
		...
		try{
			...
			if(parent_id.equals("2")){			
				PageData actPd = new PageData();
				actPd = this.getPageData();
				actPd.put("ID_", pd.getString("ROLE_ID"));
				actPd.put("NAME_", pd.getString("ROLE_NAME"));
				roleService.actAdd(actPd);
			}		
			roleService.add(pd);		
			FHLOG.save(Jurisdiction.getUsername(), "新增角色:"+pd.getString("ROLE_NAME"),"SYS_ROLE","ROLE_ID",pd.getString("ROLE_ID"));
		} catch(Exception e){
			logger.error(e.toString(), e);
	...
	public ModelAndView edit()throws Exception{
		...
		try{
			...
			Role r=roleService.getRoleById(pd.getString("ROLE_ID"));			
			if(r.getPARENT_ID().equals("2")){
				PageData actPd = new PageData();
				actPd = this.getPageData();
				actPd.put("ID_", pd.getString("ROLE_ID"));
				actPd.put("NAME_", pd.getString("ROLE_NAME"));
				roleService.actEdit(pd);
			}
			roleService.edit(pd);			
			FHLOG.save(Jurisdiction.getUsername(), "修改角色:"+pd.getString("ROLE_NAME"),"SYS_ROLE","ROLE_ID",pd.getString("ROLE_ID"));
		...
		try{
			...
			if(roleList_z.size() > 0){
				...
				if(userlist.size() > 0 || appuserlist.size() > 0){						//此角色已被使用就不能删除
					...
					Role r=roleService.getRoleById(pd.getString("ROLE_ID"));			
					if(r.getPARENT_ID().equals("2")){
						roleService.deleteActById(ROLE_ID);
					}
				   roleService.deleteRoleById(ROLE_ID);	//执行删除
				   FHLOG.save(Jurisdiction.getUsername(), "删除角色ID为:"+ROLE_ID,"SYS_ROLE","ROLE_ID",ROLE_ID);
				   errInfo = "success";
				}
			}
		} catch(Exception e){
	...
	public void saveMenuqx(@RequestParam String ROLE_ID,@RequestParam String menuIds,PrintWriter out)throws Exception{
		...
		FHLOG.save(Jurisdiction.getUsername(), "修改角色菜单权限，角色ID为:"+ROLE_ID,"SYS_ROLE","ROLE_ID",ROLE_ID);
		PageData pd = new PageData();
		try{
	...
	public void saveB4Button(@RequestParam String ROLE_ID,@RequestParam String menuIds,@RequestParam String msg,PrintWriter out)throws Exception{
		...
		FHLOG.save(Jurisdiction.getUsername(), "修改"+msg+"权限，角色ID为:"+ROLE_ID,"SYS_ROLE","ROLE_ID",ROLE_ID);
		PageData pd = new PageData();
		pd = this.getPageData();
		try{
			if(null != menuIds && !"".equals(menuIds.trim())){
