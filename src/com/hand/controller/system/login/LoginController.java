package com.hand.controller.system.login;
...
import com.hand.service.system.buttonrights.ButtonrightsManager;
import com.hand.service.system.dictionaries.DictionariesManager;
import com.hand.service.system.fhbutton.FhbuttonManager;
import com.hand.service.system.fhlog.FHlogManager;
import com.hand.service.system.loginimg.LogInImgManager;
...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		if(null != KEYDATA && KEYDATA.length == 2){
			...
			pd1 = pd;
			pd = userService.getUserByNameAndPwd(pd1);	//根据用户名和密码去读取用户信息
			if(pd == null){
				pd1.put("PASSWORD", MD5.md5(PASSWORD));
				pd = appuserService.getUserByNameAndPwd(pd1);
				}
					if(pd != null){
...
/*	@RequestMapping(value="/login_login" ,produces="application/json;charset=UTF-8")
		...
		try{
			...
			if (user != null) {
				...
				if(null == userr){
					User user1 = new User();
					user1 = user;
					user = userService.getUserAndRoleById(user1.getUSER_ID());				//通过用户ID读取用户信息和角色信息
					if(user == null){
						user = appuserService.getUserAndRoleById(user1.getUSER_ID());
					}
				...
				if(null == session.getAttribute(USERNAME + Const.SESSION_QX)){
					session.setAttribute(USERNAME + Const.SESSION_QX, this.getUQX(user));//按钮权限放到session中
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/count_title")
	@ResponseBody
	public List<PageData> getTitle() throws Exception{
		List<PageData> titleList = dictionariesService.getTitle("");
		return titleList;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/count_role")
	@ResponseBody
	public List<PageData> getRole() throws Exception{
		List<PageData> roleList = staffService.findRoleById("");
		return roleList;
	}
	/**
	 * 获取系统中的性别及总人数
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/count_sex")
	@ResponseBody
	public List<PageData> getSex() throws Exception{
		List<PageData> sexList = dictionariesService.getSex("");
		return sexList;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/count_english")
	@ResponseBody
	public List<PageData> getEnglish() throws Exception{
		List<PageData> englishList = dictionariesService.getEnglish("");
		return englishList;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/count_japanese")
	@ResponseBody
	public List<PageData> getJapanese() throws Exception{
		List<PageData> japaneseList = dictionariesService.getJapanese("");
		return japaneseList;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/count_status")
	@ResponseBody
	public List<PageData> getStatus() throws Exception{
		List<PageData> statusList = dictionariesService.getStatus("");
		return statusList;
	}
	 ...
	 * @return
	 */
	public Map<String, String> getUQX(User user){
		PageData pd = new PageData();
		Map<String, String> map = new HashMap<String, String>();
		try {
			pd.put(Const.SESSION_USERNAME, user.getUSERNAME());
			pd.put("ROLE_ID", user.getRole().getROLE_ID());//获取角色ID
